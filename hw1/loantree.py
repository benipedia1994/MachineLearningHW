...
def seperate_variables(df):
    X = df[['Age','Experience','Income','ZIP Code','Family','CCAvg','Education','Mortgage', 'Securities Account', 
...
def main():
    df = pd.read_excel("./data/banking/Bank_Personal_Loan_Modelling.xlsx","Data",index_col="ID")
    
    
    X, Y = seperate_variables(df)
    X = X/X.max()
