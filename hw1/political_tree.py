...
def main():
    df = pd.read_spss('./data/Jan20-public/Jan20public.sav')
    cat_columns = df.select_dtypes(['category']).columns
    df[cat_columns] = df[cat_columns].apply(lambda x: x.cat.codes)

#for col in df.columns: 
#    print(col) 
    trimmed = df[['respid','sex','age','educ','racecmb','chr','income','q101','q2']]

    X, Y = getLists(trimmed)
